...
- CI fetches tags and looks for tags matching `firefox-v<major.minor>.<patch>`.
...
- If the current commit already has a matching `firefox-v<major.minor>.<patch>` tag, CI reuses it (no new publish).

## Examples

- `package.json` is `1.0.0`, highest tag `firefox-v1.0.4` -> next release `1.0.5`.
